			...
			<div className={styles.overlay}>
				<div className={styles.button}>
					<span
						...
						title={t('play')}
						onClick={onPlayClick}
						data-testid={`play-button-${toCSSClassName(title)}`}
					/>
					<Text.Condensed className={styles.label} title={title}>
						{title}
					</Text.Condensed>
				</div>
			</div>
			{badges && badges.length > 0 ? (
				<div className={cx('video-badges', styles.badges)}>
					{badges}
				</div>
			) : null}
			{progress && progress < 1 ? (
				<div className={styles.progressBar}>
					<div
						...
						style={{ width: `${bucketProgress(progress) * 100}%` }}
					/>
				</div>
			) : null}
		</Monitor.ElementSize>
